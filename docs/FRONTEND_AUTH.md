...
**Dashboard (repo này):** chỉ **admin / nhân viên** đăng nhập qua **`POST /auth/login/admin`** — không dùng `POST /auth/login` (khách hàng).
...
**Gợi ý SPA:** sau khi app mở lại chỉ có `accessToken` trong memory/storage, gọi `GET /auth/me` để khôi phục state user; nếu 403 thì thử `POST /auth/refresh` rồi gọi lại `/me`.

**Triển khai dashboard:** `restoreSession` trong `authSlice` gọi `GET /auth/me` khi có `accessToken` (xem `App.jsx`).
...
3. Interceptor: nếu response **401** hoặc **403** với `message === "Token is not valid"` → thử `POST /auth/refresh` một lần rồi retry (và có thể gọi lại `/me`). **Không** refresh khi 403 do thiếu quyền staff (message chứa `Staff or admin`).
4. Đăng xuất: gọi `POST /auth/logout`, xóa token phía client, xóa state user.

**Mã nguồn:** `src/axios.js` (refresh + retry), `src/store/endpoints/auth.js` (`getAuthMe`), `src/store/slices/authSlice.js` (`restoreSession`).

## Base URL

Thay `API_BASE` bằng URL server thực tế, ví dụ `http://localhost:4000`.

Trong Vite: **`VITE_SERVER_URL`**.

Các nhóm route khác (không nằm trong `/auth`) được mount tại root tương đối, ví dụ: `/products`, `/category`, `/order`, … — xem `src/resources/router/index.route.js` (phía server).
